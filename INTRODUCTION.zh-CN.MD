...
随着 HarmonyOS 生态的快速发展，针对鸿蒙系统的 Trace 能力也变得至关重要。btrace-harmony 将在 Android、iOS 上验证过的 btrace 3.0 同步抓栈方案完整地移植到了 HarmonyOS，并针对鸿蒙系统的运行时与 ArkTS / Native 双语言栈做了深度适配，交付了**业界首个面向 HarmonyOS 的、可在真机上长期开启的高性能 Trace 工具**。

### 抓栈引擎：同步 + 异步双管齐下

**同步抓栈：ArkTS / Native 混合栈回溯**

在鸿蒙上，我们使用官方提供的 `HiDebug_Backtrace_Object` 统一进行栈回溯。它能够基于栈帧指针（`fp`）一次性 unwind 出 ArkTS 帧和 Native 帧的混合栈，通过单一 API 即可端到端贯穿应用层与系统层，无需为两种语言维护两套独立的抓栈逻辑。

**异步抓栈：兜底同步抓栈的"采样空窗"**

同步抓栈依赖业务代码不断穿过插桩点。当线程长时间停留在同一个函数体内或陷入慢系统调用时，会导致长时间不触发同步抓栈点。为此，btrace-harmony 通过定时器周期性地向目标线程发送实时信号（`tgkill`），在信号处理函数中再做一次抓栈作为兜底。通过"采样间隔节流 + 线程状态过滤"的组合策略，每个线程都按照自己的节奏被周期性采样，退出的线程与主动让出采样窗口的线程会被自动跳过。

### 慢系统调用透明代理

异步抓栈基于实时信号触发，但信号会打断当前线程正在执行的信号不安全的系统调用（如 `epoll_wait` / `recv` / `nanosleep`），导致其以 `EINTR` 提前返回，影响业务稳定性。

为此，btrace-harmony 引入 `SlowSysCallProxy`，对常见的慢系统调用做了一层透明 hook，并在进入这些调用之前**主动屏蔽采样信号**，等系统调用返回之后再恢复线程原有的信号 mask。同时在进入与退出边界各做一次主动抓栈（同步路径）。这样既彻底规避了采样信号对慢系统调用的干扰，也让 Trace 上每一段长阻塞都能被准确归因。

### 两级 Ring Buffer 流水线存储

为了应对同步抓栈的高并发写入吞吐，btrace-harmony 采用"前置并发写入 → 后置聚合存储 → 节点共享去重"的三段流水线：

1. **ConcurrentRingBuffer**：借鉴了 iOS 端多 sub-buffer 并发写入的思想，多线程并发写入原始 PC 序列时，按线程哈希分流到多个 sub buffer，通过 `try_lock` 滚动取桶显著缓解写冲突。
2. **CallstackTable**：把原始栈插入节点表进行结构去重，得到共享的栈 ID。
3. **TraceRingBuffer**：经过去重后，将包含时间戳、CPU Time、线程 ID 和栈 ID 的定长小记录，按时间相似性覆盖写入后置的环形缓冲。

这套存储架构彻底解耦了"原始栈写入"与"瘦身后存储"，使得长时间采集时内存占用保持在极低水平。
...
1. **能力补齐**：Android 端支持 Native(C/C++)层数据采集、双端支持 GPU 渲染数据的采集。
2. **线上支持**：提供在线上场景排查性能问题的能力。
3. **生态建设**：围绕 btrace 建设自动化的性能诊断工具，提供“采集即诊断”的端到端体验。
4. **大模型赋能**：引入 AI 用于 Trace 分析与问题修复，基于大模型自动定位根因并生成修复建议。

最后，欢迎大家随时探讨交流，共同打造极致的 btrace 工具！